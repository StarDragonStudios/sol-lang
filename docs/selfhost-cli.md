# Self-host command-line interface

The stage-1 compiler is split across a compiler core written in Sol and small
host launchers:
...
The Unix entry points are `selfhost/solc.sh` and `selfhost/sol.sh`. Windows uses
...
The canonical standard-library sources live under `selfhost/stdlib/` and take
