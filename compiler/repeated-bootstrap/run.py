...
BUILD = Path(os.environ.get("SOL_REPEATED_BOOTSTRAP_BUILD", COMPILER / "build" / "repeated bootstrap")).resolve()
SOURCE = COMPILER / "src" / "main.sol"
...
def inventory_paths() -> list[Path]:
    paths = list((COMPILER / "src").rglob("*.sol"))
    ...
    paths.extend(
        (
            ...
            COMPILER / "solc.bat",
            COMPILER / "solc.ps1",
            COMPILER / "native-link.sh",
            COMPILER / "native-link.bat",
        )
    )
...
def compile_stage(
    *,
    ...
    run(
        COMPILER / ("solc.bat" if IS_WINDOWS else "solc.sh") if compiler_kind == "self-hosted" else compiler,
        arguments,
...
def validate_conformance(seed: Path, core: Path, stage_name: str) -> None:
    ...
    run(
        Path(sys.executable),
        [str(COMPILER / "conformance" / "run.py")],
        environment=environment,
