...
DEFAULT_BUILD = COMPILER / "build" / "seed artifacts"
    ...
    *,
    ...
    if target.startswith("windows-"):
        for name in ("solc.bat", "solc.ps1", "sol.bat", "sol.ps1", "native-link.bat"):
            copy_file(COMPILER / name, root / "bin" / name)
    else:
        copy_file(COMPILER / "solc.sh", root / "bin" / "solc", executable=True)
        copy_file(COMPILER / "sol.sh", root / "bin" / "sol", executable=True)
        copy_file(COMPILER / "native-link.sh", root / "bin" / "native-link.sh", executable=True)
...
def verify_extracted_seed(archive: Path, root: Path, seed_environment: dict[str, str], version: str) -> None:
    ...
    forbidden = [path for path in package.rglob("*") if path.suffix.lower() in {".jar", ".class", ".java"}]
    if forbidden:
...
def build_clean_seed(
    *,
    ...
    invoke(Path(sys.executable), [str(COMPILER / "repeated-bootstrap" / "run.py")], environment=run_environment)
