...
class SeedArchiveTest(unittest.TestCase):
    ...
    def test_download_manifest_rejects_malformed_and_duplicate_entries(self) -> None:
        with tempfile.TemporaryDirectory(prefix="sol manifest ") as temporary:
            manifest = Path(temporary) / "SHA256SUMS"
            manifest.write_text("not-a-digest  bin/solc\n", encoding="utf-8")
            with self.assertRaises(trusted_seed.DownloadFailure):
                trusted_seed.manifest_entries(manifest)
            digest = "0" * 64
            manifest.write_text(f"{digest}  bin/solc\n{digest}  bin/solc\n", encoding="utf-8")
            with self.assertRaises(trusted_seed.DownloadFailure):
                trusted_seed.manifest_entries(manifest)
