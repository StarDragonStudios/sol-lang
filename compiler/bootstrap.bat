...
set "SOURCE=%COMPILER_DIR%src\main.sol"
set "BUILD_DIR=%COMPILER_DIR%build\stage1"
set "OUTPUT=%BUILD_DIR%\solc-core.exe"
set "TEST_BUILD_DIR=%COMPILER_DIR%build\tests"
set "LEXER_TEST_SOURCE=%COMPILER_DIR%src\lexer_test.sol"
set "LEXER_TEST_OUTPUT=%TEST_BUILD_DIR%\lexer_test.exe"
set "PARSER_TEST_SOURCE=%COMPILER_DIR%src\parser_test.sol"
set "PARSER_TEST_OUTPUT=%TEST_BUILD_DIR%\parser_test.exe"
set "GRAMMAR_TEST_SOURCE=%COMPILER_DIR%src\grammar_test.sol"
set "GRAMMAR_TEST_OUTPUT=%TEST_BUILD_DIR%\grammar_test.exe"
set "SEMANTIC_FOUNDATION_TEST_SOURCE=%COMPILER_DIR%src\semantic_foundation_test.sol"
set "SEMANTIC_FOUNDATION_TEST_OUTPUT=%TEST_BUILD_DIR%\semantic_foundation_test.exe"
set "SEMANTIC_ANALYSIS_TEST_SOURCE=%COMPILER_DIR%src\semantic_analysis_test.sol"
set "SEMANTIC_ANALYSIS_TEST_OUTPUT=%TEST_BUILD_DIR%\semantic_analysis_test.exe"
set "IR_TEST_SOURCE=%COMPILER_DIR%src\ir_test.sol"
set "IR_TEST_OUTPUT=%TEST_BUILD_DIR%\ir_test.exe"
set "LOWERING_TEST_SOURCE=%COMPILER_DIR%src\lowering_test.sol"
set "LOWERING_TEST_OUTPUT=%TEST_BUILD_DIR%\lowering_test.exe"
set "LLVM_TEST_SOURCE=%COMPILER_DIR%src\llvm_generation_test.sol"
set "LLVM_TEST_OUTPUT=%TEST_BUILD_DIR%\llvm_generation_test.exe"
set "LLVM_FIXTURE_SOURCE=%COMPILER_DIR%src\llvm_fixture.sol"
set "LLVM_FIXTURE_OUTPUT=%TEST_BUILD_DIR%\llvm_fixture.exe"
set "LLVM_FIXTURE_IR=%TEST_BUILD_DIR%\llvm_fixture.ll"
set "NATIVE_ARTIFACT_SOURCE=%COMPILER_DIR%src\native_artifact_fixture.sol"
...
set "CLI_FIXTURE_SOURCE=%COMPILER_DIR%fixtures\cli\main.sol"
set "CLI_FIXTURE_OUTPUT=%TEST_BUILD_DIR%\cli fixture.exe"
set "CLI_INVALID_SOURCE=%COMPILER_DIR%fixtures\cli-invalid.sol"
set "CLI_DIAGNOSTIC=%TEST_BUILD_DIR%\cli-diagnostic.txt"
set "CLI_TEST_SOURCE=%COMPILER_DIR%src\cli_test.sol"
set "CLI_TEST_OUTPUT=%TEST_BUILD_DIR%\cli_test.exe"
set "CLI_STDIN_SOURCE=%COMPILER_DIR%fixtures\cli-stdin.sol"
...
call "%COMPILER_DIR%solc.bat" --version
if errorlevel 1 exit /b %errorlevel%
call "%COMPILER_DIR%sol.bat" --version
if errorlevel 1 exit /b %errorlevel%
...
call "%COMPILER_DIR%solc.bat" --keep-intermediates --output "%CLI_FIXTURE_OUTPUT%" "%CLI_FIXTURE_SOURCE%"
if errorlevel 1 exit /b %errorlevel%
...
call "%COMPILER_DIR%solc.bat" --unknown >NUL 2>NUL
set "CLI_STATUS=!errorlevel!"
if not "!CLI_STATUS!"=="2" (
...
call "%COMPILER_DIR%solc.bat" "%CLI_INVALID_SOURCE%" -o "%TEST_BUILD_DIR%\invalid-output.exe" 2>"%CLI_DIAGNOSTIC%"
set "CLI_STATUS=!errorlevel!"
if not "!CLI_STATUS!"=="4" (
...
call "%COMPILER_DIR%sol.bat" run "%CLI_STDIN_SOURCE%" < "%CLI_STDIN_INPUT%"
set "CLI_STATUS=!errorlevel!"
