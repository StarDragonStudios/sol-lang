...
fn launch() -> int
    ...
    if !native_artifact_succeeded(artifacts) then
        ...
        if !files::write_text("selfhost/build/tests/native_fixture.ll", artifacts.llvm_ir) then
            failure = 7
        end
        if failure == 0 && !files::write_text("selfhost/build/tests/native_literals.c", artifacts.literals_c) then
            failure = 8
        end
    end
