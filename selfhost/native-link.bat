...
set "RUNTIME_DIR=%SELFHOST_DIR%..\runtime-c"
...
call "%DRIVER%" -x ir -c "%LLVM_SOURCE%" -o "%LLVM_OBJECT%"
...
if "%SOL_REPRODUCIBLE_LINK%"=="1" (
    call "%DRIVER%" "%LLVM_OBJECT%" "%RUNTIME_OBJECT%" "%LITERAL_OBJECT%" -Wl,/Brepro -o "%OUTPUT%"
) else (
