...
CONFORMANCE = SELFHOST / "conformance"
FIXTURES = CONFORMANCE / "fixtures"
BUILD = SELFHOST / "build" / "conformance suite"
...
def main() -> int:
    ...
    core = Path(os.environ.get("SOL_SELFHOST_CORE", SELFHOST / "build" / "stage1" / ("solc-core.exe" if IS_WINDOWS else "solc-core"))).resolve()
    selfhost_solc = SELFHOST / ("solc.bat" if IS_WINDOWS else "solc.sh")
    selfhost_sol = SELFHOST / ("sol.bat" if IS_WINDOWS else "sol.sh")
